 ...
 * @author ASUS
 */
public class ModalFactureFournisseur extends javax.swing.JFrame {
    private FormatUtils formater;
    private int idReceptionSelected = 0;
    private Reception recep;
    private ReceptionDAO receptionDAO;
    private CommandeFournisseurDAO commandeDAO;
    private FactureFournisseurDAO factureDAO;
    
    
    public ModalFactureFournisseur(int idReceptionSelected) {
        initComponents();
        ...
        populateFields(idReceptionSelected);
        jScrollPane3.getViewport().setBackground(Color.WHITE);
        this.idReceptionSelected = idReceptionSelected;
    }
    public ModalFactureFournisseur() {
        initComponents();
    }

    private void populateFields(int idReceptionSelected) {
        recep = receptionDAO.find(idReceptionSelected);
        
        cli_nom.setText(recep.getFournisseur().getNom()+" "+recep.getFournisseur().getPrenom());
        cli_adresse.setText(recep.getFournisseur().getAdresse());
        cli_contact.setText(recep.getFournisseur().getTel1());
        
        idLivraison.setText(String.valueOf(recep.getId()));
        refLivraison.setText(recep.getRefReception());
        dateLivraison.setText(recep.getDateReception());
        idComm.setText(String.valueOf(recep.getIdCommande()));
        refCommande.setText(commandeDAO.find(recep.getIdCommande()).getRefCommande());
        idClient.setText(String.valueOf(recep.getFournisseur().getId()));
        frais.setText(String.valueOf(recep.getFrais()));
        try {
            DateConverter.initializeDate(dateFacture, heureFacture, minuteFacture, null);
        } catch (ParseException ex) {
            Logger.getLogger(ModalFactureFournisseur.class.getName()).log(Level.SEVERE, null, ex);
        }
        
        String[] column = {"Article","Référence", "Qté commandé", "Qté colissé","PU HT","TVA","Remise (%)", "Total HT", "Total TTC"};
   
        DefaultTableModel model = new DefaultTableModel(null, column);               
               int i = 0;
           for( ReceptionDetail recepDet : recep.getReceptionDetails()){
            Object[] data = new Object[9];
            data[0] = recepDet.getProduit().getDesignation();
            data[1] = recepDet.getProduit().getRefProduit();
            data[2] = recepDet.getCommandeDetail().getQuantite();
            data[3] = recepDet.getQuantiteRecu();
            
            float ttPUHT = 0.0f ;
            float ttTVA = 0.0f;
            float ttTTC = 0.0f ;
            
            if(recepDet.getCommandeDetail().getIdProduit() == recepDet.getIdProduit()){
                
                float PUHT = recepDet.getProduit().getPUHT();
                float TVA = recepDet.getProduit().getTVA();
                float PUTTC = PUHT + (PUHT*TVA/100);
                
                ttPUHT += PUHT*recepDet.getQuantiteRecu();
                ttTVA += TVA*recepDet.getQuantiteRecu();
                ttTTC += PUTTC*recepDet.getQuantiteRecu();
                data[4] = recepDet.getProduit().getPUHT();
                data[5] = recepDet.getProduit().getTVA();
                data[6] = recepDet.getCommandeDetail().getRemise();
                data[7] = PUHT*recepDet.getQuantiteRecu();
                data[8] = PUTTC*recepDet.getQuantiteRecu();
                
            }
//                totalHT.setText(formater.formatFloat(ttPUHT));
//                totalTTC.setText(formater.formatFloat(ttTTC));
//                totalTVA.setText(formater.formatFloat(ttTVA));
//                totalNET.setText();
            
//            data[4] = recepDet.getMontant();
    ...
    public int insererFacture(){
        ...
        try {
            fac.setDateFacture(DateConverter.getInputDate(dateFacture, heureFacture, minuteFacture));
        } catch (ParseException ex) {
            Logger.getLogger(ModalFactureFournisseur.class.getName()).log(Level.SEVERE, null, ex);
        }
            fac.setDescription(description.getText());
            fac.setIdFournisseur(Integer.parseInt(idClient.getText()));
            fac.setIdReception(Integer.parseInt(idLivraison.getText()));
            fac.setMontant( Float.parseFloat( totalNetAPayerField.getText()) + recep.getFrais() );
        if(factureDAO.create(fac)){
            this.dispose();
            return factureDAO.getGeneratedKey();
        }else{
            return -1;
        }
...
//            
//            nouvelleReception.setIdProduit(1);
//            nouvelleReception.setQuantiteRecu(100);
//            nouvelleReception.setQuantiteValide(95);
//            nouvelleReception.setMontant(500.0f);
//            nouvelleReception.setIdReception(idReceptionSelected);
//            ReceptionDetailDAO livDetailDAO = DAOFactory.getReceptionDetailDAO();
//            livDetailDAO.create( nouvelleReception );
//            
//        }
//        return false;
//        
//    }
    ...
    private void initComponents() {
        ...
        jLabel17.setText("Détails de la commande");
        ...
        idComm.setFont(new java.awt.Font("Verdana", 1, 12)); // NOI18N
        idComm.setForeground(new java.awt.Color(43, 40, 54));
        idComm.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
        ...
        jPanel12Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel12Layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(idComm, javax.swing.GroupLayout.DEFAULT_SIZE, 93, Short.MAX_VALUE)
                .addContainerGap())
        );
        jPanel12Layout.setVerticalGroup(
            jPanel12Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addComponent(idComm, javax.swing.GroupLayout.DEFAULT_SIZE, 38, Short.MAX_VALUE)
        );
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel3Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel3Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanel3Layout.createSequentialGroup()
                                .addComponent(dateFacture, javax.swing.GroupLayout.PREFERRED_SIZE, 235, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(ModalFactureFournisseur.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new ModalFactureFournisseur().setVisible(true);
            }
        });
    }
    ...
    public void initEvent(ModalActionEvent event) {
