            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                        .addComponent(scrollTable, javax.swing.GroupLayout.PREFERRED_SIZE, 597, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
...
private void displayRows(int start, int end) {
    ...
    try {
        javaapp.bean.EntreeCaisse entreeCaisse = entreeCaisseDAO.find(Integer.parseInt(idSelected));
  
        if (entreeCaisse != null) {
            nomCreateur.setSelectedItem(entreeCaisse.getCreateur().getNomUtilisateur());
            reference.setText(String.valueOf(entreeCaisse.getIdSourceReference()));
            montant.setValue(entreeCaisse.getMontant());
            dateCreation.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(entreeCaisse.getDateEntree()));
            description.setText(entreeCaisse.getDescription());
            if(entreeCaisse.getTypeMouvement() != null && entreeCaisse.getTypeMouvement().getCodeType()!= 0)typeMouvement.setSelectedItem(entreeCaisse.getTypeMouvement().getType());
        }
    ...
    private void btn_deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_deleteActionPerformed
       if (JOptionPane.showConfirmDialog(null, "Etes-vous sûr de vouloir supprimer cette transaction?", "Confirmation de suppression", JOptionPane.YES_NO_OPTION) == 0) {
        try {
            ...
            entreeCaisse.setId(Integer.parseInt(idSelected));
            if (entreeCaisseDAO.delete(entreeCaisse)) {
                setEmptyForm();
    ...
    private void btn_saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_saveActionPerformed
        ...
        entreeCaisse.setDateEntree(dateFormat.format(dateCreation.getDate()) + " " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss")));
//        entreeCaisse.setIdCreateur(getIdCreateurByName((String) nomCreateur.getSelectedItem())); // Implement this method to get idCreateur
//        entreeCaisse.setIdExercice(getIdExerciceActif()); // Implement this method to get idExercice
        ...
        switch (btn_save_state) {
            ...
            case "update":
                if (JOptionPane.showConfirmDialog(null, "Voulez-vous sauvegarder les modifications?", "Confirmation de mise à jour", JOptionPane.YES_NO_OPTION) == 0) {
                    entreeCaisse.setId(Integer.parseInt(idSelected));
                    if (entreeCaisseDAO.update(entreeCaisse)) {
