...
private void displayRows(int start, int end) {
    ...
    private void tableOutputMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableOutputMouseClicked
        btn_delete.setVisible(true); // afficher le bouton supprimer
        ...
        try {
            sortieCaisseSelected = sortieCaisseDAO.find(id);
            if (sortieCaisseSelected != null) {
                reference.setText(String.valueOf(sortieCaisseSelected.getIdSourceReference()));
                montant.setValue(sortieCaisseSelected.getMontant());
                soldeIdSelected = sortieCaisseSelected.getMontant();
                nomCreateur.setSelectedItem(sortieCaisseSelected.getCreateur().getNomUtilisateur());
                dateCreation.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(sortieCaisseSelected.getDateSortie()));
                description.setText(sortieCaisseSelected.getDescription());
                if(sortieCaisseSelected.getTypeMouvement() != null && sortieCaisseSelected.getTypeMouvement().getCodeType()!= 0)typeMouvement.setSelectedItem( typeDAO.find(sortieCaisseSelected.getIdType()).getType());
            }
    ...
    private void btn_deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_deleteActionPerformed
       if(JOptionPane.showConfirmDialog(null, "Etes-vous sûr de vouloir supprimer cette transaction?","Confirmation de suppression",JOptionPane.YES_NO_OPTION) == 0){
            if (sortieCaisseDAO.delete(sortieCaisseSelected)) {
                setEmptyForm();
    ...
    private void btn_saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_saveActionPerformed
        ...
        sortieCaisse.setDateSortie(dateFormat.format(dateCreation.getDate()) + " " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss")));
        ...
        switch(btn_save_state){
            case "insert":
                ...
                break;
            case "update":
                if((solde + soldeIdSelected - sortieCaisse.getMontant()) >= 0){
...
//        refreshTable(sortieCaisseDAO.filterByDate(datetime1, query));
    ...
    private void setInputSelect(){
            nomCreateur.addItem( userDAO.getUserActive().getNomUtilisateur());
