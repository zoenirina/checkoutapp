...
public class SortieStock extends javax.swing.JPanel {
    ...
    private javaapp.bean.MouvementStock mouvementSelected;
    ...
    private void init(){
        setInputSelect();
         refreshTable(mouveStockDAO.selectOUT());
        rowcount();
...
SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
    ...
    if (refMagasinKey.getSelectedItem() != "Tous" && date1.getDate() != null && date2.getDate() != null) {
        ...
        mouvements = mouveStockDAO.filterOUTByMagasin(magDAO.findByRef(refMagasinKey.getSelectedItem().toString()).getId());
        // Update the table with the list of movements
    } else if (refMagasinKey.getSelectedItem() == "Tous" && date1.getDate() != null && date2.getDate() != null) {
        try {
            mouvements = mouveStockDAO.filterOUTBetweenDate(dateFormat.format(date1.getDate()), dateFormat.format(date2.getDate()));
        ...
        try {
            mouvements = mouveStockDAO.filterOUTByDate(dateFormat.format(date1.getDate()));
            // Update the table with the list of movements
        } catch (SQLException ex) {
            Logger.getLogger(EntreeStock.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        mouvements = mouveStockDAO.selectOUT();
    }
    refreshTable(mouvements);
    }//GEN-LAST:event_btn_searchActionPerformed
    
    public void rowcount(){
    nombre_ligne.setText("Sortie ("+ tableInput.getRowCount()+")");
    }
    
    private boolean validatedForm(){
        return refProduit.getSelectedItem().toString() != null && (int)quantite.getValue() != 0 && refMagasin.getSelectedItem().toString() != null && nomCreateur.getSelectedItem().toString() != null; 
    }
    
    private int getQuantiteStock(){

    return stockDAO.findByProduitMagasin(refProduit.getSelectedItem().toString(), refMagasin.getSelectedItem().toString()).getQuantiteStock();
    }
   
    
    private void btn_saveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_saveActionPerformed
        if(validatedForm()){
             float qteEnStock = getQuantiteStock();
            switch(btn_save_state){
                case "insert":
                    if(qteEnStock >= Integer.parseInt(quantite.getValue().toString())){
                    ...
                    if (mouveStockDAO.createOUT(mouvementStock)) {
                        ...
                        JOptionPane.showMessageDialog(null, "La quantité de ressource demandé est insuffisante pour ce mouvement! \nQuantité dsponible: "+qteEnStock,"Error", JOptionPane.OK_OPTION);
                    }                
                break;

                case "update":
                    if((qteEnStock+mouvementSelected.getQuantite() >= Integer.parseInt(quantite.getValue().toString())) ){
                    ...
                    javaapp.bean.MouvementStock mouvementStock = mouvementSelected;
                    mouvementStock.setDescription(description.getText());
                    mouvementStock.setIdProduit(produitDAO.find(refProduit.getSelectedItem().toString()).getId());
                    mouvementStock.setQuantite((int) quantite.getValue());
                    mouvementStock.setDateMouvement(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
                    ...
                    mouvementStock.setIdMagasin(magDAO.findByRef(refMagasin.getSelectedItem().toString()).getId());
                    setEmptyForm();
                    if (mouveStockDAO.updateOUT(mouvementStock)) {
                        new Toast("Modification enregistrée", pan.getLocationOnScreen().x+pan.getWidth()-320, pan.getLocationOnScreen().y+7).showtoast();
                    }
                    }    
                }else{
                    JOptionPane.showMessageDialog(null, "La quantité de ressource demandé est insuffisante pour ce mouvement! \nQuantité dsponible: "+qteEnStock ,"Erreur", JOptionPane.OK_OPTION);
                } 
                break;
            }
            refreshTable(mouveStockDAO.selectOUT());
              ...
              showMessage("Veuillez vérifier que tous les champs sont correctement remplis!","Message");
             }
...
private void setEmptyForm(){
    ...
    private void btn_deleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_deleteActionPerformed
          if (JOptionPane.showConfirmDialog(null, "Etes-vous sûr de vouloir supprimer cette ligne?", "Confirmation de suppression", JOptionPane.YES_NO_OPTION) == 0) {
            if (mouveStockDAO.delete(mouvementSelected)) {
    ...
    private void tableOutputMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableOutputMouseClicked
        ...
        mouvementSelected = mouveStockDAO.find(id);
            quantite.setValue(mouvementSelected.getQuantite());
            refMagasin.setSelectedItem(mouvementSelected.getMagasin().getRefMagasin());
            refProduit.setSelectedItem(mouvementSelected.getProduit().getRefProduit());
            nomCreateur.setSelectedItem(userDAO.find(mouvementSelected.getIdCreateur()));
            try {
                dateCreation.setDate(new SimpleDateFormat("yyyy-MM-dd").parse(mouvementSelected.getDateMouvement()));
            } catch (ParseException ex) {
                Logger.getLogger(EntreeStock.class.getName()).log(Level.SEVERE, null, ex);
            }
           description.setText(mouvementSelected.getDescription());
           
           if(mouvementSelected.getTypeMouvement() != null && mouvementSelected.getTypeMouvement().getCodeType() != 0)typeMouvement.setSelectedItem(mouvementSelected.getTypeMouvement().getCodeType());
